...
const itemsX = x => {
  return {
    "justify-start": x === POSITIONS.top,
    "justify-center": x === POSITIONS.center,
    "justify-end": x === POSITIONS.bottom,
  }
}

const itemsY = y => {
  return {
    "items-start": y === POSITIONS.top,
    "items-center": y === POSITIONS.center,
    "items-end": y === POSITIONS.bottom,
  }
}

export function Container({ className, xxs, xs, sm, md, children }) {
...
export function Inner({ className, children }) {
  const classes = classNames({
    "grid-inner": true,
    [className]: className,
  })
...
export function Row({ className, alignX, alignY, children }) {
  const classes = classNames({
    "flex flex-wrap -mx-3": true,
    "justify-start": alignX === POSITIONS.top,
    "justify-center": alignX === POSITIONS.center,
    "justify-end": alignX === POSITIONS.bottom,
    "items-start": alignY === POSITIONS.top,
    "items-center": alignY === POSITIONS.center,
    "items-end": alignY === POSITIONS.bottom,
    [className]: className,
  })
